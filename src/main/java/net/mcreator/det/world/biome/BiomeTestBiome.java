...
package net.mcreator.det.world.biome;
...
public class BiomeTestBiome extends ElementsDET.ModElement {
	@GameRegistry.ObjectHolder("det:testbiome")
	public static final BiomeGenCustom biome = null;
	public BiomeTestBiome(ElementsDET instance) {
		super(instance, 3);
	}
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockTestTopDust.block.getDefaultState().getBlock()
							|| ground == BlockWastlandDust.block.getDefaultState().getBlock())
							&& (ground2 == BlockTestTopDust.block.getDefaultState().getBlock()
									|| ground2 == BlockWastlandDust.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockWastlandDust.block.getDefaultState(), 2);
		...
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockWastlandDust.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockWastlandDust.block.getDefaultState());
		}
