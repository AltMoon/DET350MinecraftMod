...
package net.mcreator.det.world.structure;
...
public class StructureTarpit1 extends ElementsDET.ModElement {
	public StructureTarpit1(ElementsDET instance) {
		super(instance, 9);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldTestDim.DIMID) {
			...
			isNetherType = WorldTestDim.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("det", "tarpit1"));
				if (template == null)
					return;
